...
function filterDataForPrediction(data) {
  ...
  const filteredPositions = Array.isArray(data.stocksInPosition)
    ? data.stocksInPosition.map(position => ({
        symbol: position.symbol,
        quantity: position.quantity,
        avgBuyPrice: position.avgBuyPrice,
        currentPrice: position.currentPrice,
      }))
    : []; // Default to an empty array if stocksInPosition is not an array

  const filteredTransactions = Array.isArray(data.lastTransactions)
    ...
    : []; // Default to an empty array if lastTransactions is not an array
